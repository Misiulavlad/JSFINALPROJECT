// ==========================================
// KINOPOISK API LOGIC (FINAL VERSION)
// ==========================================
...
export const kinopoiskAPI = {
    ...
    searchByKeyword: async (query) => {
        ...
        try {
            const response = await fetch(url, {
                method: 'GET',
                headers: { 'X-API-KEY': kinopoiskAPI.key, 'Content-Type': 'application/json' },
            });
    ...
    openFilmModal: async (filmId) => {
        ...
        try {
            ...
            // Определяем лучшую ссылку для воспроизведения
            let finalUrl = "";

            // 1. Пробуем найти прямую ссылку на трейлер
            ...
            // 2. Если прямой ссылки нет, формируем ссылку поиска (запасной вариант)
            if (!finalUrl) {
                // Используем encodeURIComponent для безопасной вставки названия
                finalUrl = `https://www.youtube.com/embed?listType=search&list=trailer+${encodeURIComponent(name)}&autoplay=1`;
            }

            // Рисуем кнопку "СМОТРЕТЬ"
            const controls = document.createElement('div');
            controls.id = 'videoControls';
            controls.style.padding = '40px';
            controls.style.textAlign = 'center';
            
            // Вставляем кнопку. При клике передаем найденную ссылку в функцию playVideo
            controls.innerHTML = `
    ...
    // 4. Запуск видео (вызывается при клике на кнопку)
    playVideo: (url) => {
        const player = document.getElementById('modalPlayer');
        const controls = document.getElementById('videoControls');
        
        // Вставляем ссылку и показываем плеер
        player.src = url;
